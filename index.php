...
// Totais comissões (exemplo)
$totais_comissao = $pdo->query("
    SELECT SUM(CASE WHEN status=0 THEN 1 ELSE 0 END) AS pendente,
           SUM(CASE WHEN status=1 THEN 1 ELSE 0 END) AS pago
    FROM salario_comissao
")->fetch(PDO::FETCH_ASSOC);

// Dados últimos 7 dias para mini-gráficos
...
$dados_comissao_dia = $pdo->query("
    SELECT DATE(dt_ref_fin) as dia, 
           SUM(CASE WHEN status=0 THEN 1 ELSE 0 END) as nao_pago, 
           SUM(CASE WHEN status=1 THEN 1 ELSE 0 END) as pago
    FROM salario_comissao
    WHERE dt_ref_fin >= CURDATE() - INTERVAL 6 DAY
    GROUP BY DATE(dt_ref_fin)
    ORDER BY DATE(dt_ref_fin)
")->fetchAll(PDO::FETCH_ASSOC);

$cor_boletos = ($totais_boletos['nao_pago'] > 0) ? 'danger pulse' : 'success';
$cor_pedidos = ($totais_pedidos['nao_pago'] > 0) ? 'danger pulse' : 'success';
$cor_comissao = ($totais_comissao['pendente'] > 0) ? 'warning pulse' : 'secondary';
?>
...
.sidebar { min-height: 100vh; width: 220px; position: fixed; top: 0; left: 0; background: #343232d7; color: #fff; padding-top: 1rem; }
.sidebar a, .sidebar button.accordion-button { color:#fff; text-decoration:none; display:flex; align-items:center; padding:0.3rem 0.8rem; border-radius:5px; margin:0.1rem 0; font-size:0.9rem; }
.sidebar a:hover, .sidebar button.accordion-button:hover { background: rgba(255,255,255,0.15);}
.sidebar h4 { font-size:1rem; margin-bottom:1rem; text-align:center; }
...
.card-mini-chart { height: 40px; }
.card-small h5 { font-size:0.9rem; }
.card-small h2 { font-size:1.5rem; margin-bottom:0; }
</style>
</head>
    ...
    <div class="accordion-item bg-transparent border-0">
        ...
        <div id="collapseFunc" class="accordion-collapse collapse" data-bs-parent="#sidebarAccordion">
            <div class="accordion-body p-0">
                <a href="funcionario/listar.php">Listar Funcionários</a>
                <a href="funcionario/inserir.php">Novo Funcionário</a>
...
<nav class="navbar bg-primary navbar-dark d-md-none">
    <div class="container-fluid">
        ...
        <span class="navbar-brand mb-0 h1">Sistema de Gestão</span>
    </div>
</nav>
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasMenu">
    ...
    <div class="offcanvas-body p-0">
        <div class="accordion" id="offcanvasAccordion">
            <a href="funcionario/listar.php" class="d-block p-2">Listar Funcionários</a>
...
<div class="content">
    ...
    <div class="bg-white shadow rounded p-3 mb-4">
        <h3 class="mb-2">Bem-vindo ao Sistema de Gestão</h3>
        <p>Use este painel para monitorar boletos, pedidos, fornecedores e funcionários.</p>
    </div>
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-4 mb-2">
            <div class="card card-small border-0 shadow-sm text-white bg-<?= $cor_boletos ?>">
                <div class="card-body">
                    <h5>Boletos não pagos</h5>
                    <h2><?= $totais_boletos['nao_pago'] ?></h2>
                    <canvas id="miniChartBoletos" class="card-mini-chart mt-1"></canvas>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-4 mb-2">
            <div class="card card-small border-0 shadow-sm text-white bg-<?= $cor_comissao ?>">
                <div class="card-body">
                    ...
                    <h2><?= $totais_comissao['pendente'] ?></h2>
                    <canvas id="miniChartComissao" class="card-mini-chart mt-1"></canvas>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card border-0 shadow-sm p-3">
                <h5 class="mb-2">Comparativo Pagos x Não Pagos</h5>
...
new Chart(ctxMain, {
    type: 'bar',
    data: {
        labels: ['Boletos', 'Pedidos', 'Comissões'],
        datasets: [
            { label: 'Pago', data: [<?= $totais_boletos['pago'] ?>, <?= $totais_pedidos['pago'] ?>, <?= $totais_comissao['pago'] ?>], backgroundColor: '#198754' },
            { label: 'Não pago', data: [<?= $totais_boletos['nao_pago'] ?>, <?= $totais_pedidos['nao_pago'] ?>, <?= $totais_comissao['pendente'] ?>], backgroundColor: '#dc3545' }
        ]
    },
    options: { indexAxis: 'y', responsive: true, plugins: { legend: { position: 'bottom' } } }
});

function gerarMiniChart(id, dados, corPago, corNaoPago) {
    ...
    new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [
                { label: 'Pago', data: pagos, borderColor: corPago, backgroundColor: corPago.replace('1)', '0.2)'), tension: 0.3 },
                { label: 'Não pago', data: abertos, borderColor: corNaoPago, backgroundColor: corNaoPago.replace('1)', '0.2)'), tension: 0.3 }
            ]
        },
...
gerarMiniChart('miniChartBoletos', <?= json_encode($dados_boletos_dia) ?>, '#198754', '#dc3545');
gerarMiniChart('miniChartPedidos', <?= json_encode($dados_pedidos_dia) ?>, '#198754', '#dc3545');
gerarMiniChart('miniChartComissao', <?= json_encode($dados_comissao_dia) ?>, '#0d6efd', '#ffc107');
</script>
